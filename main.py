...
def domain_banner():
    banner = Text("Found Subdomains - CRT.SH", justify='center', style='green bold')
    print(Panel(banner))
    print()
...
def crtsh(domain):
	host = "https://crt.sh/?q="
	req_url = host+domain+"&output=json" # host url (https://crt.sh/?q=<domain>&output=json)
	req = requests.get(req_url) # requesting url
	found_data = req.text # loading data as text
	data_dict = json.loads(found_data) # loading json data in python dictionary
...
if __name__ == "__main__":
	...
	parser.add_argument('-d', "--domain", help="Specify Target Domain", type=str, required=True)
	parser.add_argument('-r', "--raw", help="Raw Output", action="store_true")
	parser.add_argument('-o', "--output", help="Write output on a file", type=str)
